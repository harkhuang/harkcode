...
//	printf("%s\n",c->rem.io.buf);
	...
	if (strcmp(env_name, "REQUEST_METHOD") == 0) {
		...
		return (c->uri);
	} else if (strcmp(env_name, "AUTH_INFO") == 0) {

		if(c->ch.auth.v_vec.len > 0){
			((char *)c->ch.auth.v_vec.ptr)[c->ch.auth.v_vec.len] = '\0';
			return &c->ch.auth.v_vec.ptr[6];		
		}
...
//////////////gengkeming add start
#if 0
	var_name_len = var_len;

	memset(buff_uri,0,sizeof(buff_uri));
	memset(buff_varname,0,sizeof(buff_varname));
	strcpy(buff_uri,buf);
	strcpy(&buff_varname[1],var);

	buff_varname[0] = '?';
	pvalue = strstr(buff_uri,buff_varname);
	if( NULL != pvalue )
	{
		pvalue += var_name_len + 1;
