 ...
 * $Id: shttpd.c,v 1.1.1.1 2014/10/30 05:47:30 wangruxia.oristartech.com Exp $
 */
...
//static int
int
_get_path_info(struct conn *c, char *path, struct stat *stp)
{
	char	*p, *e;
...
{
	char		path[URI_PATH_MAX], buf[1024], *root;
	...
	} else if (_get_path_info(c, path, &st) != 0) {
...
parse_http_request(struct conn *c)
{
	...
	req_len = c->rem.headers_len =
	    _shttpd_get_headers_len(s, io_data_len(&c->rem.io));	/// 1   ///2
	...
	*c->headers++ = '\0';
	...
	if (sscanf(p, "HTTP/%lu.%lu%n",
		...
		_shttpd_strlcpy(c->uri, (char *) start, uri_len + 1);		/// 2
...
//static void
void
_read_stream(struct stream *stream)
{
	int	n, len;
...
//	    (int) io_data_len(&c->rem.io), io_data(&c->rem.io)));
	...
	if ((_shttpd_current_time > c->expire_time) ||
		...
		connection_desctructor(&c->link);		/// 3
}
		...
		fd_set *read_set, fd_set *write_set)
{
	...
	LL_FOREACH(&worker->connections, lp) {		
		c = LL_ENTRY(lp, struct conn, link);	///2
		
		/* If there is a space in remote IO, check remote socket */
		if (io_space_len(&c->rem.io))
			add_to_set(c->rem.chan.fd, read_set, max_fd);    ///2  ///3
...
//gengkeming		DBG(("FD_SET(%d) (listening)", l->sock));
...
//	printf("num_workers(ctx):%d\n",num_workers(ctx));

	if (shttpd_join(ctx, &read_set, &write_set, &max_fd))
		milliseconds = 0;			/// 3


	if ((select_ret = do_select(max_fd, &read_set, &write_set, milliseconds)) < 0)
		return;;
