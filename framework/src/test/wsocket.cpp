...
CWSocket::CWSocket()
{
  ...
  SetInvalidSocket(m_hsocket);
	// ×é²¥µØÖ·²»³õÊ¼»¯Ôì³É³ÌÐòÕ¼ÓÃµÚÒ»¸öÌ×½Ó×Ö,Á½ÌìÕÒµ½Õâ¸öBUG,¹æ·¶Ñ½£¬¹æ·¶!!!!
...
int CWSocket::ReciveData(char *pbuf, int nlen, int ntimeout, bool bdataarriret)
{
  ...
  while (nReaded < nlen)
  {
    ...
    if (nRet == 0)
    {
      ...
      if (m_bstopsocket 
		  || (m_hquit_event !=NULL && WaitForSingleObject(m_hquit_event, 0) == WAIT_OBJECT_0) )     //Lanq add m_hQuitEventÍË³ö
      {//Ó¦¸ÃÉèÖÃ×´Ì¬·ñ?
        m_nerr = -10;
...
int CWSocket::SetProxy(int nproxytype, char *lpszproxyaddress, int nport, char *lpszuser, char *lpszpwd, int nconnecttimeout)
{
  if (nproxytype < WSOCKET_PROXY_DIRECT || nproxytype >= WSCOKET_PROXY_MAX)
  {
	...
	if (lpszproxyaddress && strlen(lpszproxyaddress) > 0 && !IsIpAddr(lpszproxyaddress))
	{
		...
		{
			//LoginTrace("½âÎöÓòÃû'%s'Ê§°Ü£¬´íÎóºÅ£º%d",szHostIP,WSAGetLastError());
			return -2;
		}
	}
...
*/
	...
	//if(strstr(szBuf, "HTTP/1.0 200 OK") == NULL) //Á¬½Ó²»³É¹¦
	//{
	//   Close();
	//   return -4;
	//} 
...
bool CWSocket::__Connect(char *lpszserver , int nport, int nconnecttimeout)
{
  ...
  while (nct < m_nconnecttimeout)
  {
	...
	while (nct < m_nconnecttimeout)
	{
		...
		/* 5.ÉèÖÃµÈ´ýÊ±¼ä£¬Ê¹ÓÃselectº¯ÊýµÈ´ýÕýÔÚºóÌ¨Á¬½ÓµÄconnectº¯Êý£¬ÕâÀïÐèÒªËµÃ÷µÄÊÇÊ¹ÓÃ
		     select¼àÌýsocketÃèÊö·ûÊÇ·ñ¿É¶Á»òÕß¿ÉÐ´£¬Èç¹ûÖ»¿ÉÐ´£¬ËµÃ÷Á¬½Ó³É¹¦£¬¿ÉÒÔ½øÐÐÏÂÃæµÄ
		     ²Ù×÷¡£Èç¹ûÃèÊö·û¼È¿É¶ÁÓÖ¿ÉÐ´£¬·ÖÎªÁ½ÖÖÇé¿ö£¬µÚÒ»ÖÖÇé¿öÊÇsocketÁ¬½Ó³öÏÖ´íÎó£¨²»Òª
		     ÎÊÎªÊ²Ã´£¬ÕâÊÇÏµÍ³¹æ¶¨µÄ£¬¿É¶Á¿ÉÐ´Ê±ºòÓÐ¿ÉÄÜÊÇconnectÁ¬½Ó³É¹¦ºóÔ¶³ÌÖ÷»ú¶Ï¿ªÁËÁ¬
		     ½Óclose(socket)£©£¬µÚ¶þÖÖÇé¿öÊÇconnectÁ¬½Ó³É¹¦£¬socket¶Á»º³åÇøµÃµ½ÁËÔ¶³ÌÖ÷»ú·¢ËÍ
		     µÄÊý¾Ý¡£ÐèÒªÍ¨¹ýconnectÁ¬½Óºó·µ»Ø¸øerrnoµÄÖµÀ´½øÐÐÅÐ¶¨£¬»òÕßÍ¨¹ýµ÷ÓÃ 
		     getsockopt(sockfd,SOL_SOCKET,SO_ERROR,&error,&len); º¯Êý·µ»ØÖµÀ´ÅÐ¶ÏÊÇ·ñ·¢Éú´íÎó£¬
		     ÕâÀï´æÔÚÒ»¸ö¿ÉÒÆÖ²ÐÔÎÊÌâ£¬ÔÚsolarisÖÐ·¢Éú´íÎó·µ»Ø-1£¬µ«ÔÚÆäËûÏµÍ³ÖÐ¿ÉÄÜ·µ»Ø0.ÎÒÊ×
		     ÏÈ°´unixÍøÂç±à³ÌµÄÔ´Âë½øÐÐÊµÏÖ¡£ÈçÏÂ£º*/
...
#if defined(WIN32) || defined(WIN64)
//¼ÓÈë¶à²¥£¬udpÐ­Òé£¬¿Í»§¶Ë<1>SetAddressPort<2>Join
...
bool CWSocket::Join(DWORD dwflag)
{
	...
	OutputDebugString(szdebugbuf);
		...
		//socket(AF_INET,	SOCK_RAW,	IPPROTO_ICMP); ÈçÒªÉèÖÃtimeout,±ØÐë°Ñ×îºóÒ»¸ö²ÎÊýÉèÖÃÎª WSA_FLAG_OVERLAPPED 
		//WINDOW TIMEOUTÎªDWORD£¬ LINUX Îª struct timeval 
...
bool CWSocket::CheckConnectTime(char *pszAddress , int nPort, int nconnecttimeout,int &nrealconect_tickcount)
{
	...
	DWORD dwbegin = 0;    // ·þÎñÆ÷µÄ GetTickCount() Ê±¼ä
	DWORD dwend   = 0;    // ·þÎñÆ÷µÄ GetTickCount() Ê±¼ä
